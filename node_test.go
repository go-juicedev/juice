...
func TestSelectFieldAliasNode_Accept(t *testing.T) {
	drv := driver.MySQLDriver{}
	translator := drv.Translator()          // Not strictly used by SelectFieldAliasNode but good practice for Node tests
	emptyParams := newGenericParam(H{}, "") // Not used by SelectFieldAliasNode but good practice

	tests := []struct {
		name          string
		items         SelectFieldAliasNode
		expectedQuery string
		// SelectFieldAliasNode does not produce args, so no expectedArgs needed
	}{
		{
			name:          "EmptyNode",
			items:         SelectFieldAliasNode{},
		...
		{
			name: "SingleItem_WithAlias",
			items: SelectFieldAliasNode{
				{column: "user_name", alias: "userName"},
			},
			expectedQuery: "user_name AS userName",
		},
		{
			name: "SingleItem_AliasSameAsColumn",
			items: SelectFieldAliasNode{
				{column: "id", alias: "id"},
			},
			expectedQuery: "id", // Should not add "AS id"
		},
		{
			name: "MultipleItems_Mixed",
			items: SelectFieldAliasNode{
				{column: "id"},
			...
			expectedQuery: "id, first_name AS firstName, last_name AS lastName, age",
		},
		{
			name: "MultipleItems_AllWithAliases",
			items: SelectFieldAliasNode{
				{column: "col1", alias: "c1"},
				{column: "col2", alias: "c2"},
			},
			expectedQuery: "col1 AS c1, col2 AS c2",
		},
		{
			name: "MultipleItems_AllWithoutAliases",
			items: SelectFieldAliasNode{
				{column: "field_a"},
				{column: "field_b"},
			},
			expectedQuery: "field_a, field_b",
		},
		{
			name: "ItemWithEmptyAliasString", // Explicitly empty alias string should be treated as no alias
			items: SelectFieldAliasNode{
				{column: "field_x", alias: ""},
			},
			expectedQuery: "field_x",
		},
	}
...
func TestValuesNode_Accept(t *testing.T) {
	drv := driver.MySQLDriver{}
	translator := drv.Translator()
	...
	tests := []struct {
		...
		expectedQuery string
		expectedArgs  []any
		expectError   bool // Though ValuesNode.Accept itself doesn't seem to return errors directly other than from NewTextNode
	}{
		{
			name:          "EmptyValuesNode",
			values:        ValuesNode{},
		...
		{
			name: "SingleValueItem",
			values: ValuesNode{
				{column: "name", value: "#{name}"},
			},
			params:        newGenericParam(H{"name": "Alice"}, ""),
			expectedQuery: "(name) VALUES (?)",
			expectedArgs:  []any{"Alice"},
		},
		{
			name: "MultipleValueItems",
			values: ValuesNode{
				{column: "id", value: "#{id}"},
				{column: "name", value: "#{name}"},
				{column: "age", value: "#{age}"},
			},
			params:        newGenericParam(H{"id": 1, "name": "Bob", "age": 30}, ""),
			expectedQuery: "(id, name, age) VALUES (?, ?, ?)",
			expectedArgs:  []any{1, "Bob", 30},
		},
		{
			name: "ValueItemWithDotNotationParam",
			values: ValuesNode{
				{column: "user_id", value: "#{user.id}"},
			},
			params:        newGenericParam(H{"user": map[string]any{"id": 123}}, ""),
			expectedQuery: "(user_id) VALUES (?)",
			expectedArgs:  []any{123},
		},
		{
			name: "ValueItemWithMissingParam", // NewTextNode within Accept will handle this
			values: ValuesNode{
				{column: "data", value: "#{missing_data}"},
			},
			params:      newGenericParam(H{}, ""),
			expectError: true, // Error from underlying TextNode due to missing parameter
		},
		{
			name: "ValuesWithMixedPlaceholdersAndLiteralsInValues",
			values: ValuesNode{
				{column: "col1", value: "#{val1}"},
				{column: "col2", value: "LITERAL_VALUE"}, // This literal will be part of the query string passed to NewTextNode
				{column: "col3", value: "#{val3}"},
			},
			params:        newGenericParam(H{"val1": "data1", "val3": "data3"}, ""),
			expectedQuery: "(col1, col2, col3) VALUES (?, LITERAL_VALUE, ?)",
			expectedArgs:  []any{"data1", "data3"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				// Can check specific error message if needed, e.g., "parameter missing_data not found"
				return
			}
